...
Если нужно собрать все элементы, попавшие в одну группу в массив, то можно воспользоваться функцией `.collect_list()`
```python
df.groupBy("color").agg(F.collect_list("x")).show()
# output
+-----+--------------------+
|color|     collect_list(x)|
+-----+--------------------+
|  red|     [602, 797, 137]|
| blue|[688, 128, 509, 6...|
|green|          [141, 482]|
...
Если нужно собрать все элементы, попавшие в одну группу в массив, то можно воспользоваться функцией `.collect_set()`
```python
df.groupBy("color").agg(F.collect_set("x")).show()
# output
+-----+--------------------+
|color|     collect_set(x)|
+-----+--------------------+
