...
Функция `.ntile()` -- оконная функция, которая возвращает идентификационный номер группы. Например, `.ntile(4)` разобьет записи на 4 группы и для первой группы каждая строка получит индекс 1, каждая строка группы 2 -- индекс 2 и т.д.

Пусть есть кадр данных с 10 записями. Требуется имеющийся набор данных разбить на 5 частей, сгруппировать по каждой части и вычислить для каждой части среднее. 

ВАЖНО! Функция `.ntile()`, будучи оконной, работает в связке с `Window.partitionBy("...")` и ожидает, что атрибут, по которому выполняется партиционирование, содержит подгруппы -- например, атрибут `color` мог бы содержать подгруппы `red`, `green`, `blue`. Вот для каждой такой подгруппы выполнялось разбиение на заданное количество партиций. Если нужно все записи кадра данных разбить на указанное число партиций (то есть у атрибута партиционирования нет подгрупп), придется создать атрибут с _константым значением_. Если не указать имя столбца, по которому выполняется партиционирование, то получим предупреждение
```
24/06/13 01:06:10 WARN WindowExec: No Partition Defined for Window operation! Moving all data to a single partition, this can cause serious performance degradation.
```
```python
df.select(
    "id", "x", "y",
    F.ntile(5) \
        .over(Window.partitionBy("group") \  # over делает функцию оконной!!!
        .orderBy("x") \  # упорядочиваем по атрибуту `x`
        .alias("ntile")  # вешаем псевдоним
).groupBy("ntile").agg(
    F.mean("x"),
    F.mean("y"),
    F.stddev("y")
