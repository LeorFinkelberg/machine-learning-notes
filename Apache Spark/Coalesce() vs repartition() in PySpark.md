...
Для примера, если мы работаем на кластере и данные разбиты на несколько партиций, а мы хотим записать один файл, то можно воспользоваться методом `.coalesce()`
```python
df.coalesce(1).write.format("csv").option("|").save("output_dir_csv")
```

В директорию `output_dir_csv` запишется один CSV-файл вида `part-00000-5a...csv`. Без метода `.coalesce()` в эту директорию записалось бы столько CSV-файлов, сколько было заданно партиций. Например, если при создании Spark-сессии ведущий узел был определен как `.master("local[*]")`, а на машине 8 ядер, то данные будут разбиваться на 8 партиций и следовательно при записи файлов на диск мы поличим 8 CSV-файлов. Это можно проверить с помощью терминальной операции (метода-действия) `.saveAsText()`.

К слову, если есть необходимость сохранить данные с партиционирование по какому-то категориальному атрибуту, то можно сделать так
```python
# Атрибут `color` содержит три уникальных значения: `red`, `green` и `blue`
df.write.partitionBy("color").format("csv").option("delimeter", "|").save("output_csv")
```

Здесь в директории `output_csv` появятся поддиректории с именами `color=blue`,  `color=red` и `color=green`. Каждая из которых будет содержать CSV-файлы вида `part-0001-50b...csv`.
